...
/* iathook.c: log who calls QString::fromUtf16 with a bad pointer. */
...
BOOL WINAPI DllMain(HINSTANCE instance, DWORD reason, LPVOID reserved)
{
    ...
    if (reason == DLL_PROCESS_ATTACH) {
        ...
        /* Instrument the Qt helper the client dies in, so the next crash names
         * its caller instead of just an address inside Qt5Core. */
        uuyc_fromutf16_hook_install();
